...
// Build a Cypher relationship pattern string for use in MATCH clauses.
// Examples:
//   relationshipPattern(null, 'all', null)        returns '-[*]-'
//   relationshipPattern(['KNOWS'], 'out', 5)      returns '-[:KNOWS*..5]->'
//   relationshipPattern(['KNOWS','LOVES'], 'in', 3) returns '<-[:KNOWS|LOVES*..3]-'
//   relationshipPattern(null, 'out', 1)           returns '-[*..1]->'
//   relationshipPattern('KNOWS', 'all', null)     returns '-[:KNOWS*]-'

function relationshipPattern (types, direction, maxDepth) {
  var typeStr = '';

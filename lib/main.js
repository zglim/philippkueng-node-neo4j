...
/*	Build the `{ node, created }` result for `mergeNode` from a cypher response.
	`created` is derived from the query statistics (nodes_created) so we never have
	to write a marker property onto the node itself. */
...
function mergeResult (res) {
	...
	if (res) {
		if (res.stats) {
			created = res.stats.nodes_created >= 1;
		}
...
/*	Merge a Node (find-or-create / upsert)
	...
	Find a node by one or more `labels` and a set of `identityProperties`. When no
	matching node exists it is created, otherwise the existing node is reused. This
	wraps a Cypher MERGE so callers don't have to hand-assemble the query.

	* `labels`              String|Array[String]   e.g.: 'User' or ['User', 'Student'] (at least one non-empty label)
	* `identityProperties`  Object                 the properties that make up the MERGE identity, e.g.: { userid: '124' } (non-empty)
	* `options`             Object                 (Optional)
	    * `options.onCreate`  Object   properties written ONLY when the node is created, e.g.: { createdAt: 1419 }
	    * `options.onMatch`   Object   properties written when an existing node is matched (and also kept fresh on create), e.g.: { lastSeen: 1420 }

	The callback receives `(err, result)` where `result` is:
	    { node: <the node data, including its _id>, created: <Boolean> }
	`result.created` is `true` when a new node was inserted and `false` when an
	existing node was reused, so the caller can branch on the outcome.

	Note: properties in `identityProperties` end up in the MERGE pattern, while
	`onCreate` / `onMatch` are applied through ON CREATE SET / ON MATCH SET. This
	keeps the matching condition limited to the real identity fields.

	Examples:
	  mergeNode('User', { userid: '124' }, function (err, res) {
	    // res.created === true on first call, false afterwards
	    // res.node._id, res.node.userid
	  });

	  mergeNode(['User', 'Admin'], { email: 'sam@example.com' }, {
	    onCreate: { createdAt: Date.now() },
	    onMatch: { lastSeen: Date.now() }
	  }, function (err, res) { ... });								*/

var mergeNode = function(labels, identityProperties, options, callback) {
	...
	var val = new Validator();
	val.requiredLabels(labels)
		.identityProperties(identityProperties)
		.optionalProperties(onCreate, 'onCreate')
		.optionalProperties(onMatch, 'onMatch');

	if (val.hasErrors) {
		return callback(val.error(), null);
	}

	var merge = cypher.mergeProperties('data', identityProperties, onCreate, onMatch);

	var query = 'MERGE (data' + cypher.labels(labels) + ' {' + merge.pattern + '})';
	if (merge.onCreate !== '') {
		query += ' ON CREATE SET ' + merge.onCreate;
	}
	if (merge.onMatch !== '') {
		query += ' ON MATCH SET ' + merge.onMatch;
	}
	query += ' RETURN data';

	// Ask for statistics so we can tell whether the node was created or matched
	// without writing a marker property onto the node itself.
	this.cypherQuery(query, merge.properties, true, function(err, res) {
		if (err) {
			return callback(err, null);
		}
		callback(null, mergeResult(res));
	});
};
// Create an alias
Neo4j.prototype.mergeNode = mergeNode;
Neo4j.prototype.findOrCreateNode = mergeNode;

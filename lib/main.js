...
/* Read multiple Nodes / Relationships by id -------- */

/*	Read multiple Nodes at once given an array of node ids.
	A higher level helper so callers don't have to hand-craft a batch payload
	or loop over readNode themselves.

	* The input keeps its order: result[i] always corresponds to node_ids[i],
	  regardless of which underlying request finishes first.
	* A node that is found is returned as its data object (with the _id property),
	  reusing the exact same parsing as readNode (addNodeId).
	* A node id that doesn't exist (or isn't a valid id) resolves to `false`, the
	  same convention readNode uses, so the caller can tell hits from misses
	  without the whole call failing.

	Examples:
		readNodesByIds([1, 2, 3], callback);
			returns [ { _id: 1, ... }, { _id: 2, ... }, { _id: 3, ... } ]
		readNodesByIds([1, 123456789], callback);
			returns [ { _id: 1, ... }, false ]   // second id was not found
		readNodesByIds([], callback);
			returns [] */

Neo4j.prototype.readNodesByIds = function(node_ids, callback) {
	readEntitiesById.call(this, 'readNode', 'Node', node_ids, callback);
};

/*	Read multiple Relationships at once given an array of relationship ids.
	Behaves like readNodesByIds but reuses readRelationship, so every found
	relationship keeps the regular _id / _start / _end / _type normalisation.
	Missing or invalid ids resolve to `false`.

	Examples:
		readRelationshipsByIds([10, 11], callback);
			returns [ { _id: 10, _start: .., _end: .., ... }, { _id: 11, ... } ]
		readRelationshipsByIds([10, 123456789], callback);
			returns [ { _id: 10, ... }, false ]   // second id was not found */

Neo4j.prototype.readRelationshipsByIds = function(relationship_ids, callback) {
	readEntitiesById.call(this, 'readRelationship', 'Relationship', relationship_ids, callback);
};
...
/*	Validate that a value is usable as a Neo4j id.
	Neo4j ids are non-negative integers, so accept whole numbers and numeric
	strings and reject everything else (null, undefined, objects, '12a', ...). */

var isValidId = function(id) {
	if (typeof id === 'number') {
		return isFinite(id) && id >= 0 && id % 1 === 0;
	}
	if (typeof id === 'string') {
		return /^[0-9]+$/.test(id);
	}
...
/*	Shared implementation for readNodesByIds / readRelationshipsByIds.

	`reader` is the name of the single-item read method to reuse ('readNode' or
	'readRelationship') so the per-item parsing (_id, _start, _end, ...) stays in
	a single place instead of being duplicated for the batch case.

	Results are gathered with step's group(), which keeps them in the order the
	ids were supplied no matter which request finishes first. Ids that are not
	valid are turned into a `false` miss without firing a request, so a malformed
	id can never crash the batch and the positional alignment is preserved. */

var readEntitiesById = function(reader, entityName, ids, callback) {
	var that = this;

	if (!Array.isArray(ids)) {
		return callback(new Error('The "ids" parameter for reading ' + entityName + 's in batch should be an array.'), null);
	}
	...
	step(
		function readAll() {
			...
			ids.forEach(function(id) {
				var done = group();
				if (isValidId(id)) {
					that[reader](id, done); // reuse the single-item read and its parsing
				} else {
					done(null, false); // not a usable id -> miss, no request, keep order
				}
			});
		},

...
/*	Internal helper to validate and normalize path query options.
	Returns { error: Error } on invalid input, or { options: { types, direction, maxDepth } } when valid. */

Neo4j.prototype._validatePathOptions = function(startNodeId, endNodeId, options) {
	var val = new Validator();
	val.nodeId(startNodeId);

	// Validate endNodeId separately with a clearer message
	if(typeof endNodeId !== 'number' || endNodeId % 1 !== 0 || endNodeId < 0) {
		val.addError('"endNodeId" should be a positive integer.');
	}

	var direction = 'all';
	if(options.direction !== undefined && options.direction !== null) {
	...
	if(options.types !== undefined && options.types !== null) {
		if(typeof options.types === 'string') {
			types = [options.types];
			val.relationshipTypes(options.types);
	...
	return { options: { types: types, direction: direction, maxDepth: maxDepth } };
};

/*	Query the shortest path between two nodes.
	Uses Cypher shortestPath under the hood.
	Options:
		types       String|Array[String]    Optional. Filter by relationship type(s).
		direction   String                  Optional. 'all' (default), 'in', or 'out'.
		maxDepth    Integer                 Optional. Maximum traversal depth (>= 1).

	Examples:
		readShortestPath(10, 20, callback);
		readShortestPath(10, 20, { types: 'KNOWS' }, callback);
		readShortestPath(10, 20, { types: ['KNOWS', 'WORKS_WITH'], direction: 'out', maxDepth: 5 }, callback);

	Returns the Cypher result object with columns and data.
	When no path exists, data will be an empty array (not an error). */

Neo4j.prototype.readShortestPath = function(startNodeId, endNodeId, options, callback) {
	if(typeof options === 'function') {
		callback = options;
		options = {};
	}

	var validated = this._validatePathOptions(startNodeId, endNodeId, options);
	if(validated.error) {
		return callback(validated.error, null);
	}

	var opts = validated.options;
	var relPattern = cypher.relationshipPattern(opts.types, opts.direction, opts.maxDepth);

	var query = 'MATCH (a), (b) WHERE id(a) = {startId} AND id(b) = {endId} ' +
		'MATCH p = shortestPath((a)' + relPattern + '(b)) RETURN p';

	this.cypherQuery(query, { startId: startNodeId, endId: endNodeId }, function(err, result) {
		if(err) {
...
/*	Query all paths between two nodes up to a given depth.
	Uses Cypher variable-length path matching.
	Options:
		types       String|Array[String]    Optional. Filter by relationship type(s).
		direction   String                  Optional. 'all' (default), 'in', or 'out'.
		maxDepth    Integer                 Required. Maximum traversal depth (>= 1).

	Examples:
		readAllPaths(10, 20, { maxDepth: 3 }, callback);
		readAllPaths(10, 20, { types: ['KNOWS'], direction: 'out', maxDepth: 5 }, callback);

	Returns the Cypher result object with columns and data.
	When no path exists, data will be an empty array (not an error). */

Neo4j.prototype.readAllPaths = function(startNodeId, endNodeId, options, callback) {
	if(typeof options === 'function') {
		callback = options;
		options = {};
	}

	// maxDepth is required for readAllPaths to avoid unbounded traversal
	if(options.maxDepth === undefined || options.maxDepth === null) {
		options = Object.assign({}, options, { maxDepth: undefined });
	}
	...
	if(validated.options.maxDepth === null) {
		return callback(new Error('"maxDepth" is required for readAllPaths and should be a positive integer (>= 1).'), null);
	}
	...
	var query = 'MATCH (a), (b) WHERE id(a) = {startId} AND id(b) = {endId} ' +
		'MATCH p = (a)' + relPattern + '(b) RETURN p';

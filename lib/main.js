...
/*	Internal helper. Normalise the overloaded `cypherQuery` arguments and return
	a consistent `{ params, options, callback }` triple. Supports:
		cypherQuery(query, callback)
		cypherQuery(query, params, callback)
		cypherQuery(query, params, include_stats|options, callback)		*/

function normalizeCypherArguments(params, options, callback) {
	if (typeof params === 'function') {
		return { params: null, options: null, callback: params };
	}
	if (typeof options === 'function') {
		return { params: params, options: null, callback: options };
	}
	return { params: params, options: options, callback: callback };
}

/*	Internal helper. Turn the optional third argument of `cypherQuery` (either a
	legacy boolean `include_stats` flag or an options object) into a normalised
	`{ include_stats, graph }` object. */

function parseCypherOptions(options) {
	var includeStats = false;
	if (typeof options === 'boolean') {
		includeStats = options;            // legacy `include_stats` flag
	} else if (options && typeof options === 'object') {
		includeStats = !!options.include_stats;
	}
	return { include_stats: includeStats, graph: cypherOptionsWantGraph(options) };
}

/*	Internal helper. Decide whether a graph structured result was requested. */

function cypherOptionsWantGraph(options) {
	...
	return Array.isArray(options.resultDataContents) &&
		options.resultDataContents.indexOf('graph') !== -1;
}

/*	Internal helper. Build an Error out of the `errors` array returned by the
	transactional endpoint, or return null when there is no error. */

function neoTransactionError(body) {
	if (body && body.errors && body.errors.length > 0) {
		var neoError = body.errors[0];
		return new Error('Error when running the graph cypher query against neo4j.\n' +
			neoError.code + ': ' + neoError.message);
	}
	return null;
}
...
/*	Run a Cypher Query.
	...
	By default the legacy row endpoint (/db/data/cypher) is used and the original
	behaviour is preserved: nodes get their `_id` added and the (possibly
	flattened) `data` / `columns` are returned.

	The third argument is optional and fully backwards compatible:
		cypherQuery(query, callback)
		cypherQuery(query, params, callback)
		cypherQuery(query, params, include_stats, callback)   // include_stats: boolean
		cypherQuery(query, params, options, callback)          // options: object

	Supported options:
		{ include_stats: true }                 // same as the legacy boolean flag
		{ graph: true }                         // request a graph structured result
		{ resultDataContents: ['row','graph'] } // explicit, mirrors the transaction API

	When a graph result is requested the query is executed through the
	transactional endpoint with `resultDataContents: ['row', 'graph']` and the
	result is normalised to:
		{
			columns: [ ... ],       // the returned columns
			data: [ [ ... ], ... ], // the raw `row` arrays (multi column safe)
			nodes: [ ... ],         // every node, flattened & de-duplicated, each with _id
			relationships: [ ... ]  // every edge, flattened & de-duplicated, each with
			                        // _id / _start / _end / _type
		}
	The `nodes` and `relationships` arrays are ready to be handed to a graph
	visualisation or to further processing without walking the raw response.

	See `graphQuery` for a shorthand of the graph variant.		*/

Neo4j.prototype.cypherQuery = function(query, params, options, callback) {
	var that = this;

	// Normalise the overloaded arguments while staying backwards compatible.
	var args = normalizeCypherArguments(params, options, callback);
	params = args.params;
	callback = args.callback;
	var resolved = parseCypherOptions(args.options);

	var val = new Validator();
	if (val.query(query).hasErrors) {
		return callback(val.error(), null);
	}

	// Graph structured results are only available through the transactional
	// endpoint, so route there and let `graphQuery` normalise the response.
	if (resolved.graph) {
		return that.graphQuery(query, params, callback);
	}
	...
	request
		.post(that.url + '/db/data/cypher' + (resolved.include_stats ? '?includeStats=true' : ''))
		.set(this.header)
...
/*	Run a Cypher Query and return a graph structured result.

	Shorthand for `cypherQuery(query, params, { graph: true }, callback)`.

		graphQuery(query, callback)
		graphQuery(query, params, callback)

	Returns (see `cypherQuery` for details):
		{ columns: [...], data: [...], nodes: [...], relationships: [...] }		*/

Neo4j.prototype.graphQuery = function(query, params, callback) {
	var that = this;
	...
	request
		.post(that.url + '/db/data/transaction/commit')
		.set(that.header)
		.set('Content-Type', 'application/json')
		.set('X-Stream', true)
		.send({ statements: [statement] })
		.on('error', callback)
		.end(function(result) {
			switch(result.statusCode) {
				case 200:
					var graphError = neoTransactionError(result.body);
					if (graphError) {
						return callback(graphError, null);
					}
					callback(null, parser.parseGraphResult(result.body));
					break;
				case 404:
					callback(null, null);
					break;
				default:
					callback(new Error('HTTP Error ' + result.statusCode + ' when running the graph cypher query against neo4j.'), null);
			}
		});
};
...
/*	Normalise a single value returned inside a cypher row.
	A REST relationship object (it has `self`, `start`, `end` and `data`) is
	normalised through `addRelationshipId` so it gets `_id / _start / _end /
	_type`. Everything else (nodes, scalars, paths, ...) keeps the previous
	behaviour and is handled by `addNodeId`. */

Neo4j.prototype.addCypherId = function(item, callback) {
	if (item && item.self && item.data && item.start && item.end &&
		/\/relationship\/[0-9]+$/.test(item.self)) {
		return this.addRelationshipId(item, callback);
	}
	return this.addNodeId(item, callback);
};

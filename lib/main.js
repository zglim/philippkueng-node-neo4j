...
/*	Internal helper shared by readShortestPath and readPaths.
	Validates the node ids and the (optional) `types`, `direction` and `maxDepth`
	options, builds the relationship pattern once via cypher.relationshipPattern
	and delegates the assembled query to cypherQuery.
	`buildQuery(pattern)` turns the relationship pattern into the full cypher statement.

	When no path matches, cypherQuery returns the library's regular empty result
	({ columns: [...], data: [] }) instead of an error - i.e. "no path" is not a hard
	error. Malformed arguments (illegal node id, illegal direction, empty types array,
	invalid maxDepth, ...) are reported through the callback as an Error. */

var queryPath = function(that, from_id, to_id, options, callback, buildQuery) {
	if (typeof options === 'function') {
		callback = options;
		options = {};
	}
	...
	var pattern = cypher.relationshipPattern({
		types: options.types,
	...
	that.cypherQuery(buildQuery(pattern), { fromId: from_id, toId: to_id }, callback);
};

/*	Find the shortest path between two nodes (referenced by their node id).

	The result has the same shape as a `RETURN path` cypher query, i.e.
	result.columns is ['path'] and result.data is an array with a single path object
	({ start, end, length, nodes: [...], relationships: [...] }) when a path exists,
	or an empty array when there is no path between the two nodes.

	Examples:
	Shortest path using any relationship type, any direction and any depth:
		readShortestPath(1, 5, callback);
	Constrain the relationship type(s), the direction and the maximum depth:
		readShortestPath(1, 5, {
			types: ['KNOWS', 'LIKES'], // optional, a single string or an array of strings
			direction: 'out',          // optional, 'all' (default), 'in' or 'out'
			maxDepth: 4                // optional, limits the path length to 4 hops
		}, callback); */

Neo4j.prototype.readShortestPath = function(from_id, to_id, options, callback) {
	queryPath(this, from_id, to_id, options, callback, function(pattern) {
		return 'MATCH (from), (to), path = shortestPath((from)' + pattern + '(to)) ' +
			'WHERE id(from) = {fromId} AND id(to) = {toId} ' +
			'RETURN path';
	});
};

/*	Find all paths between two nodes (referenced by their node id).

	Accepts the same options as readShortestPath (`types`, `direction`, `maxDepth`)
	and returns one path object per matching path, or an empty result when there is
	no path. Setting `maxDepth` is strongly recommended to keep the traversal bounded.

	Example:
		readPaths(1, 5, { types: 'KNOWS', direction: 'out', maxDepth: 3 }, callback); */

Neo4j.prototype.readPaths = function(from_id, to_id, options, callback) {
	queryPath(this, from_id, to_id, options, callback, function(pattern) {
		return 'MATCH path = (from)' + pattern + '(to) ' +
			'WHERE id(from) = {fromId} AND id(to) = {toId} ' +
			'RETURN path';
	});
};

...
// Wrap a callback so that an array result is reduced to its first element.
// A miss (empty array or a missing index) yields `false`, consistent with
// readNode/readRelationship rather than throwing an error.
function firstOrFalse(callback) {
...
/*	Low level query against a manual index, shared by node and relationship
	indexes. Returns an array of the matching items (with their ids added).
	An existing index with no matching entry returns an empty array, a missing
	index returns false. */

Neo4j.prototype.queryItemsFromIndex = function(args, callback) {
	var that = this;

	if (!isNonEmptyString(args.indexName) || !isNonEmptyString(args.indexKey) || !isIndexValue(args.indexValue)) {
		return callback(new Error('"indexName" and "key" should be non-empty strings and "value" should be provided.'), null);
	}

	request
		.get(that.url + '/db/data/index/' + args.indexType + '/' + args.indexName + '/' + args.indexKey + '/' + args.indexValue)
		.set(this.header)
		.end(function(result) {
			switch(result.statusCode) {
				case 200:
					if (result.body && result.body.length >= 1) {
						if (args.indexType === 'relationship') {
							that.addRelationshipIdForArray(result.body, callback);
						} else {
							that.addNodeIdForArray(result.body, callback);
						}
					} else {
						callback(null, result.body); // Index exists but nothing matched.
					}
					break;
				case 404:
					callback(null, false); // Index doesn't exist.
					break;
				default:
					callback(new Error('HTTP Error ' + result.statusCode + ' when querying an Index'), null);
			}
		});
};

/*	Query a node index by key/value.
	Example:
		queryNodeIndex('the_index_name', 'a_key', 'a_value', callback);
		returns an array of matching nodes (empty array if nothing matched). */

Neo4j.prototype.queryNodeIndex = function(indexName, key, value, callback) {
	this.queryItemsFromIndex({
		indexType: 'node',
		indexName: indexName,
		indexKey: key,
		indexValue: value
...
/*	Query a relationship index by key/value.
	Example:
		queryRelationshipIndex('the_index_name', 'a_key', 'a_value', callback);
		returns an array of matching relationships (empty array if nothing matched). */

Neo4j.prototype.queryRelationshipIndex = function(indexName, key, value, callback) {
	this.queryItemsFromIndex({
		indexType: 'relationship',
		indexName: indexName,
		indexKey: key,
...
/*	Read a single node from an index by key/value.
	Returns the first matching node or `false` when nothing matched. */

Neo4j.prototype.readNodeFromIndex = function(indexName, key, value, callback) {
	this.queryNodeIndex(indexName, key, value, firstOrFalse(callback));
};

/*	Read a single relationship from an index by key/value.
	Returns the first matching relationship or `false` when nothing matched. */

Neo4j.prototype.readRelationshipFromIndex = function(indexName, key, value, callback) {
	this.queryRelationshipIndex(indexName, key, value, firstOrFalse(callback));
};

/* Remove an Item from an Index ---------- */

/*	Low level removal of an indexed item, shared by node and relationship
	indexes. The key and value are optional so the three common REST forms are
	supported: remove all entries for an item, remove entries for a given key,
	or remove one specific key/value entry. */

Neo4j.prototype.removeItemFromIndex = function(args, callback) {
	var path = this.url + '/db/data/index/' + args.indexType + '/' + args.indexName;

	if (isIndexValue(args.indexKey)) {
		path += '/' + args.indexKey;
		if (isIndexValue(args.indexValue)) {
			path += '/' + args.indexValue;
		}
	}
	path += '/' + args.itemId;
	...
	request
		...
		.end(function(result) {
			switch(result.statusCode) {
				case 200:
				case 204:
					callback(null, true); // Item removed from the index.
					break;
				case 404:
					callback(null, false); // Item or index doesn't exist.
					break;
				default:
					callback(new Error('HTTP Error ' + result.statusCode + ' when removing an Item from an Index'), null);
			}
		});
};

// Normalize the flexible (itemId, indexName, [key], [value], callback) signature
// used by the public removal wrappers and validate the shared arguments.
function removeFromIndex(that, indexType, itemId, indexName, key, value, callback) {
	...
	var idName = indexType === 'node' ? '"nodeId"' : '"relationshipId"';
	if (!isPositiveInteger(itemId)) {
		return callback(new Error(idName + ' should be a positive integer.'), null);
	}
	if (!isNonEmptyString(indexName)) {
		return callback(new Error('"indexName" should be a non-empty string.'), null);
	}

	that.removeItemFromIndex({
		indexType: indexType,
		itemId: itemId,
		indexName: indexName,
		indexKey: key,
...
/*	Remove a node from an index.
	Examples:
		...
		removeNodeFromIndex(node_id, 'the_index_name', 'a_key', 'a_value', callback);
		// remove every entry of the node for a given key
		removeNodeFromIndex(node_id, 'the_index_name', 'a_key', callback);
		// remove every entry of the node from the index
		removeNodeFromIndex(node_id, 'the_index_name', callback);
	Returns true when the removal succeeded, false when the item or index doesn't exist. */

Neo4j.prototype.removeNodeFromIndex = function(nodeId, indexName, key, value, callback) {
	removeFromIndex(this, 'node', nodeId, indexName, key, value, callback);
};

/*	Remove a relationship from an index.
	Same forms as removeNodeFromIndex but for relationship indexes. */

Neo4j.prototype.removeRelationshipFromIndex = function(relationshipId, indexName, key, value, callback) {
	removeFromIndex(this, 'relationship', relationshipId, indexName, key, value, callback);
};
...
/* Add node_id for an array of nodes */

Neo4j.prototype.addNodeIdForArray = function(nodes, callback) {
	var that = this;
	step(

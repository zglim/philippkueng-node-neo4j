...
/*	Merge a Node (find-or-create / upsert)
	Given one or more labels and a set of identity properties, find a matching
	node or create a new one. Additional properties can be set exclusively on
	creation (createProperties) or exclusively when an existing node is matched
	(updateProperties).

	Returns an object { node, created } where `node` is the node data (including
	_id) and `created` is a boolean indicating whether the node was newly created
	(true) or an existing node was matched (false).

	Parameters:
	* `labels`             String|Array[String]  e.g.: 'User' or ['User', 'Admin']
	* `identityProperties` Object                Properties used in the MERGE match condition
	...
	* `callback`           Function              callback(err, { node, created })
	...
	Examples:
		db.mergeNode('User', { email: 'a@b.com' }, { name: 'Alice', age: 30 }, callback);
		db.mergeNode(['User','Admin'], { email: 'a@b.com' }, { name: 'Alice' }, { lastLogin: 'today' }, callback);
*/

Neo4j.prototype.mergeNode = function(labels, identityProperties, createProperties, updateProperties, callback) {
	// Handle flexible arity: createProperties and updateProperties are both optional
	if (typeof createProperties === 'function') {
		callback = createProperties;
		createProperties = {};
		updateProperties = {};
	} else if (typeof updateProperties === 'function') {
	...
	// --- Validation ---
	var val = new Validator();
	val.labels(labels).properties(identityProperties);

	if (val.hasErrors) {
		return callback(val.error(), null);
	}

	// Labels must not be an empty array
	if (labels instanceof Array && labels.length === 0) {
		return callback(new Error('"Labels" should contain at least one non-empty string.'), null);
	}

	// Identity properties must have at least one key
	if (!identityProperties || Object.keys(identityProperties).length === 0) {
		return callback(new Error('"identityProperties" should be an object with at least one property.'), null);
	}
	...
	if (createProperties && typeof createProperties !== 'object') {
		return callback(new Error('"createProperties" should be an object.'), null);
	}
	if (updateProperties && typeof updateProperties !== 'object') {
		return callback(new Error('"updateProperties" should be an object.'), null);
	}
	...
	// Identity properties for the MERGE clause
	var identityParts = [];
	for (var key in identityProperties) {
		if (identityProperties.hasOwnProperty(key)) {
			var pName = prefix + (++i);
			identityParts.push(key + ': {' + pName + '}');
			params[pName] = identityProperties[key];
		}
	}

	var query = 'MERGE (data' + cypher.labels(labels) + ' {' + identityParts.join(', ') + '})';

	// ON CREATE SET: properties set only when the node is newly created
	var createParts = [];
	for (var cKey in createProperties) {
		if (createProperties.hasOwnProperty(cKey)) {
			// Skip keys already in identity (already set by MERGE)
			if (identityProperties.hasOwnProperty(cKey)) continue;
			var cPName = prefix + (++i);
			createParts.push('data.' + cKey + ' = {' + cPName + '}');
			params[cPName] = createProperties[cKey];
		}
	}

	// Always set a marker on create to detect whether the node was newly created
	var markerCreatedName = prefix + (++i);
	createParts.push('data._merge_marker = {' + markerCreatedName + '}');
	params[markerCreatedName] = '__created__';

	query += ' ON CREATE SET ' + createParts.join(', ');

	// ON MATCH SET: properties set only when an existing node is matched
	var updateParts = [];
	for (var uKey in updateProperties) {
		if (updateProperties.hasOwnProperty(uKey)) {
			// Skip keys already in identity (shouldn't change identity)
			if (identityProperties.hasOwnProperty(uKey)) continue;
			var uPName = prefix + (++i);
			updateParts.push('data.' + uKey + ' = {' + uPName + '}');
			params[uPName] = updateProperties[uKey];
		}
	}

	if (updateParts.length > 0) {
		query += ' ON MATCH SET ' + updateParts.join(', ');
	}

	query += ' RETURN data';

	this.cypherQuery(query, params, function(err, res) {
		if (err) {
			return callback(err, null);
		}

		var nodeData = res.data[0];
		var created = nodeData && nodeData._merge_marker === '__created__';

		// Clean up the internal marker property
		if (nodeData) {
			delete nodeData._merge_marker;
		}

		callback(null, { node: nodeData, created: created });
	});
};

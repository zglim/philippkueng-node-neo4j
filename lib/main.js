...
/*  Run a Cypher query against Neo4j.
    ...
    Signatures (backward compatible):
        ...
        cypherQuery(query, params, include_stats, callback)
        cypherQuery(query, params, options, callback)

    The optional third argument can be:
      - a boolean   -> treated as include_stats (legacy behavior, hits /db/data/cypher)
      - a function  -> treated as the callback (legacy behavior)
      - an object   -> treated as options:
            { graph: true, includeStats: true }
        When `graph` is true, the query is routed through the transaction
        endpoint (/db/data/transaction/commit) with
        resultDataContents: ['row', 'graph'] so the caller receives both
        traditional row data and a normalized graph structure in the same
        response.

    Return value (row mode - default):
        {
            columns: [ 'a', 'b' ],
            data:    [ ... ]           // flat array for single column, nested for multiple
        }

    Return value (graph mode, options.graph === true):
        {
            columns: [ 'p', 'n' ],
            data:    [ ... ],          // same shape as row mode
            graph: {
                nodes:         [ { _id, _labels, ...properties } ],
                relationships: [ { _id, _start, _end, _type, ...properties } ]
            }
        }
*/
...
Neo4j.prototype.cypherQuery = function(query, params, include_stats, callback) {
    ...
    // Argument normalization: detect callback in different positions
    if (typeof params === 'function') {
        callback = params;
        params = null;
        include_stats = false;
    } else if (typeof include_stats === 'undefined') {
        include_stats = false;
    } else if (typeof include_stats === 'function') {
        callback = include_stats;
        include_stats = false;
    }

    // Detect the new options object: non-null, non-array plain object
    var options = {};
    if (include_stats !== null && typeof include_stats === 'object' && !Array.isArray(include_stats)) {
        options = include_stats;
        include_stats = options.includeStats || false;
    }

    // Graph mode: route through the transaction endpoint
    if (options.graph) {
        return this._cypherQueryGraph(query, params, !!include_stats, callback);
    }
...
/*  Internal: execute a cypher query in graph mode via the transaction endpoint.
    Returns both row and graph data, normalized with _id / _start / _end / _type
    fields using the same parser helpers used elsewhere in the library. */

Neo4j.prototype._cypherQueryGraph = function(query, params, includeStats, callback) {
    var that = this;
    var statement = {
        statement: query,
        resultDataContents: ['row', 'graph']
    ...
    var url = that.url + '/db/data/transaction/commit';
    if (includeStats) {
            ...
            switch(result.statusCode) {
                case 200:
                    if (result.body.errors && result.body.errors.length > 0) {
                        var firstError = result.body.errors[0];
                        return callback(new Error(firstError.code + ': ' + firstError.message), null);
                    }
                    if (result.body.results && result.body.results.length >= 1) {
                        var parsed = that._parseGraphResult(result.body.results[0]);
                        callback(null, parsed);
                    } else {
                        callback(null, { columns: [], data: [], graph: { nodes: [], relationships: [] } });
                    }
                    break;
                case 404:
                    callback(null, null);
                    break;
                default:
                    var msg = 'HTTP Error ' + result.statusCode + ' when running the cypher query against neo4j.';
...
/*  Internal: parse a single result block from the transaction endpoint into
    a normalized structure containing `columns`, `data` (row format, matching
    the legacy /db/data/cypher shape) and `graph` with flattened nodes and
    relationships. Duplicates (same node/relationship id appearing in multiple
    result rows) are merged so each entity appears at most once. */

Neo4j.prototype._parseGraphResult = function(txResult) {
    var columns = txResult.columns || [];
    var rows = [];
    var nodeMap = {};
    var relMap = {};

    var dataItems = txResult.data || [];
    for (var i = 0; i < dataItems.length; i++) {
        var item = dataItems[i];

        // Row data: keep the same shape as legacy cypherQuery
        // (flatten when there is exactly one column)
        if (columns.length === 1) {
            rows.push(item.row[0]);
        } else {
            rows.push(item.row);
        }

        // Graph data: merge nodes and relationships, dedup by id
        if (item.graph) {
            var gNodes = item.graph.nodes || [];
            for (var n = 0; n < gNodes.length; n++) {
                var gNode = gNodes[n];
                if (!nodeMap[gNode.id]) {
                    nodeMap[gNode.id] = parser.parseGraphNode(gNode);
                }
            }
            var gRels = item.graph.relationships || [];
            for (var r = 0; r < gRels.length; r++) {
                var gRel = gRels[r];
                if (!relMap[gRel.id]) {
                    relMap[gRel.id] = parser.parseGraphRelationship(gRel);
                }
            }
        }
    }
    ...
    return {
        ...
        graph: {
            ...
            relationships: Object.keys(relMap).map(function(k) { return relMap[k]; })
        }
    };
};

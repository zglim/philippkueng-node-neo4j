...
/* Batch Read Nodes by IDs --------
   Given an array of node ids (positive integers),
   returns an array of the same length preserving the original order.
   For each id that exists, the corresponding element is the node object (with _id).
   For each id that does not exist, the corresponding element is null.
   Examples:
       readNodesByIds([1, 2, 3], callback);
           returns [ {_id:1, name:'A'}, {_id:2, name:'B'}, {_id:3, name:'C'} ]
       readNodesByIds([1, 999999], callback);
           returns [ {_id:1, name:'A'}, null ]
       readNodesByIds([], callback);
           returns []																*/

Neo4j.prototype.readNodesByIds = function(ids, callback) {
	var that = this;

	// Validate input
	if (!(ids instanceof Array)) {
		return callback(new Error('"ids" should be an array of positive integers.'), null);
	}
	...
	for (var i = 0; i < ids.length; i++) {
		...
		if (typeof id !== 'number' || id % 1 !== 0 || id < 0) {
			return callback(new Error('Each id in "ids" should be a positive integer. Invalid id at index ' + i + ': ' + id), null);
		}
	}

	// Build batch payload, mapping batch id -> original index
	var batchPayload = [];
	...
	this.batchQuery(batchPayload, function(err, results) {
		if (err) {
			return callback(err, null);
		}

		// batchQuery returns null for HTTP 404 (e.g. all items fail at HTTP level)
		if (!results) {
			var allNull = new Array(ids.length);
			for (var k = 0; k < ids.length; k++) { allNull[k] = null; }
			return callback(null, allNull);
		}

		// Build a map from batch id -> response body
		var resultMap = {};
		for (var r = 0; r < results.length; r++) {
			resultMap[results[r].id] = results[r];
		}

		// Build ordered result array, using addNodeId for parsing
		var orderedResults = new Array(ids.length);
		var pending = ids.length;
		var failed = false;

		for (var idx = 0; idx < ids.length; idx++) {
			(function(index) {
				var item = resultMap[index];
				if (item && item.status === 200 && item.body) {
					that.addNodeId(item.body, function(err, node) {
						if (failed) return;
						if (err) {
							failed = true;
							return callback(err, null);
						}
						orderedResults[index] = node;
						if (--pending === 0) callback(null, orderedResults);
					});
				} else {
					// Not found or error status -> null
...
/* Batch Read Relationships by IDs --------
   Given an array of relationship ids (positive integers),
   returns an array of the same length preserving the original order.
   For each id that exists, the corresponding element is the relationship object (with _id, _start, _end, _type).
   For each id that does not exist, the corresponding element is null.
   Examples:
       readRelationshipsByIds([1, 2], callback);
           returns [ {_id:1, _start:10, _end:11, _type:'KNOWS'}, {_id:2, _start:10, _end:12, _type:'LIKES'} ]
       readRelationshipsByIds([1, 999999], callback);
           returns [ {_id:1, _start:10, _end:11, _type:'KNOWS'}, null ]
       readRelationshipsByIds([], callback);
           returns []																*/

Neo4j.prototype.readRelationshipsByIds = function(ids, callback) {
	var that = this;
	...
	this.batchQuery(batchPayload, function(err, results) {
		...
		// batchQuery returns null for HTTP 404
		if (!results) {
			var allNull = new Array(ids.length);
			for (var k = 0; k < ids.length; k++) { allNull[k] = null; }
			return callback(null, allNull);
		...
		// Build ordered result array, using addRelationshipId for parsing
		var orderedResults = new Array(ids.length);
		var pending = ids.length;
		var failed = false;
		...
		for (var idx = 0; idx < ids.length; idx++) {
			(function(index) {
				var item = resultMap[index];
				if (item && item.status === 200 && item.body) {
					that.addRelationshipId(item.body, function(err, rel) {
						if (failed) return;
						if (err) {
							failed = true;
							return callback(err, null);
						}
						orderedResults[index] = rel;
						if (--pending === 0) callback(null, orderedResults);
					});
				} else {
					// Not found or error status -> null
